...
export class KuaishouPlatformAdapter extends BasePlatformAdapter {
  readonly platform = 'kuaishou' as const
  readonly config = PLATFORM_CONFIGS.kuaishou
  ...
  async login(storageState?: unknown): Promise<LoginResult> {
    ...
    await this.page.goto(PLATFORMS.kuaishou.loginUrl)
  ...
  private async getNickname(): Promise<string> {
    if (!this.page) return ''
    try {
      const element = await this.page.$('.user-name')
