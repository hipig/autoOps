...
- [SidebarMenuButtonChild.vue](file://src/renderer/src/components/ui/sidebar/SidebarMenuButtonChild.vue)
- [SidebarMenuSub.vue](file://src/renderer/src/components/ui/sidebar/SidebarMenuSub.vue)
- [utils.ts](file://src/renderer/src/components/ui/sidebar/utils.ts)
- [index.ts](file://src/renderer/src/components/ui/sidebar/index.ts)
</cite>
...
## 简介
本文件面向 AutoOps 的全新侧边栏导航系统，系统性阐述其设计理念、导航模式与用户交互方式；详细说明各子组件的属性、事件与插槽；覆盖折叠/展开、路由集成、菜单高亮、嵌套菜单、与主内容区域的协调机制、响应式行为、动态菜单与权限控制、工具提示、快捷键与键盘导航等关键能力，并提供可直接定位到源码位置的参考路径，便于快速查阅与二次开发。
...
MenuButtonChild["SidebarMenuButtonChild.vue"]
MenuSub["SidebarMenuSub.vue"]
end
...
- [SidebarMenuButtonChild.vue:1-36](file://src/renderer/src/components/ui/sidebar/SidebarMenuButtonChild.vue#L1-L36)
- [SidebarMenuSub.vue:1-22](file://src/renderer/src/components/ui/sidebar/SidebarMenuSub.vue#L1-L22)
...
- [index.ts:1-61](file://src/renderer/src/components/ui/sidebar/index.ts#L1-L61)
...
- SidebarMenuButtonChild：菜单按钮基础实现，支持变体和尺寸
- SidebarMenuSub：子菜单容器，支持嵌套菜单
- utils.ts：上下文定义、样式变量、Cookie 名称与快捷键常量
- index.ts：组件导出入口，包含变体定义
...
### SidebarMenuButtonChild（菜单按钮基础实现）
- 职责
  - 提供菜单按钮的基础实现
  - 支持变体和尺寸系统
  - 处理激活状态和数据属性
...
**章节来源**
- [SidebarMenuButtonChild.vue:1-36](file://src/renderer/src/components/ui/sidebar/SidebarMenuButtonChild.vue#L1-L36)
- [index.ts:38-61](file://src/renderer/src/components/ui/sidebar/index.ts#L38-L61)
...
SidebarMenuButton --> SidebarMenuButtonChild : "组合"
...
- 组件优化
  - 使用 reactiveOmit 避免不必要的属性传递
  - 条件渲染工具提示，减少 DOM 元素数量
...
- 嵌套菜单显示问题
  - 检查 SidebarMenuSub 的样式类是否正确应用
  - 确认子菜单项的 data-sidebar 属性设置
...
## 结论
AutoOps 的全新侧边栏组件体系以"上下文 + 容器 + 组合子组件"为核心，实现了灵活的折叠/展开、响应式布局、路由集成与高亮、工具提示与快捷键支持。通过清晰的职责划分与数据属性驱动样式，既保证了易用性，也兼顾了扩展性与可维护性。新增的菜单按钮变体系统和嵌套菜单支持进一步增强了组件的灵活性和可定制性。
...
### 菜单按钮变体系统
- 使用 sidebarMenuButtonVariants 定义按钮样式
- 支持 default/outline 变体和 default/sm/lg 尺寸
- 参考路径：[index.ts:38-61](file://src/renderer/src/components/ui/sidebar/index.ts#L38-L61)

### 嵌套菜单实现
- 使用 SidebarMenuSub 创建子菜单容器
- 通过 SidebarMenuItem 包装子菜单项
- 支持图标模式下的自动隐藏
- 参考路径：[SidebarMenuSub.vue:1-22](file://src/renderer/src/components/ui/sidebar/SidebarMenuSub.vue#L1-L22)